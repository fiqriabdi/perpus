...
$kolom = ''; // Kolom pencarian (judul, penulis, atau isbn)

// Proses pencarian ketika form dikirim
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $keyword = mysqli_real_escape_string($conn, $_POST['keyword']); // Hindari SQL injection
    $kolom = $_POST['kolom'];
    ...
    if ($kolom === 'judul') {
        ...
        // ❗Masih fallback karena tidak ada kolom isbn di database
        $query = "SELECT * FROM books WHERE judul LIKE '$keyword%' OR penulis LIKE '%$keyword%'";
    } else {
        $query = "SELECT * FROM books WHERE judul LIKE '$keyword' OR penulis LIKE '$keyword'";
    }

    $hasil = mysqli_query($conn, $query); // Jalankan query
}
?>
...
<head>
  ...
  <!-- Library CSS dari CDN -->
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font/bootstrap-icons.css" rel="stylesheet">
  <link href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css" rel="stylesheet"/>
  <link href="../assets/css/style.css" rel="stylesheet"> <!-- Gaya lokal -->
</head>
<body>
...
<nav class="navbar navbar-expand-lg fixed-top shadow-sm">
  <div class="container">
    <a class="navbar-brand fw-bold text-primary" href="./dashboard.php">
      <img src="../assets/img/logo.png" alt="Logo" width="40" class="me-2"> Tiga Sisi
    </a>
      ...
      <div class="card h-100 shadow-sm">
        ...
        <div class="card-body d-flex flex-column">
          ...
          <div class="mt-auto d-flex justify-content-between">
            <a href="detail.php?id=<?= $row['id_buku'] ?>" class="btn btn-sm btn-outline-primary">
              <i class="bi bi-info-circle"></i> Detail
            </a>
            <?php if ($row['stok'] > 0) : ?>
              <form method="post" action="../proses/proses_pinjam.php">
                <input type="hidden" name="id_buku" value="<?= $row['id_buku'] ?>">
                <button type="submit" name="pinjam" class="btn btn-sm btn-success">
